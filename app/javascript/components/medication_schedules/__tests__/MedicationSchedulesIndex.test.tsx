...
import { MedicationSchedulesIndex } from '../MedicationSchedulesIndex';
import { MedicationSchedule } from '../../../types/medicationSchedule';

const renderWithRouter = () => {
  return render(
    <MemoryRouter>
      <MedicationSchedulesIndex />
    </MemoryRouter>
  );
};
...
describe('MedicationSchedulesIndex', () => {
  beforeEach(() => {
  ...
  it('displays loading state initially', () => {
    ...
    renderWithRouter();
    expect(screen.getByText('Loading medication schedules...')).toBeInTheDocument();
  });

  it('fetches and displays medication schedules', async () => {
  ...
  it('calls fetch with correct URL', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/v1/medication_schedules', {
        headers: {
  ...
  it('displays correct links for each schedule', async () => {
    ...
    expect(viewLinks[0]).toHaveAttribute('href', '/medication_schedules/1');
    expect(editLinks[0]).toHaveAttribute('href', '/medication_schedules/1/edit');
  });
